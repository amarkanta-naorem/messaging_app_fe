  ...
  return (
    <div className={`fixed inset-0 z-60 ${isOpen ? 'pointer-events-auto' : 'pointer-events-none'}`}>
       ...
       <div className={`absolute inset-y-0 right-0 w-full max-w-md bg-white shadow-2xl transform transition-transform duration-300 ease-in-out flex flex-col ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}>
          {/* Header */}
          <div className="flex flex-col p-4 border-b border-slate-100 bg-[#008069] text-white shrink-0">
             ...
             <p className="text-xs text-white/80">Select people to add to this group</p>
          </div>
          ...
          <div className="p-3 border-b border-slate-100 bg-slate-50 shrink-0">
             <div className="relative">
                ...
                <input 
                  ...
                  className="w-full pl-10 pr-4 py-2 rounded-lg border border-slate-200 focus:outline-none focus:border-[#008069] text-sm"
                  value={searchQuery}
                   ...
                   {filteredContacts.map((contact, index) => {
                      ...
                      return (
                        <div key={contact.id}>
                            ...
                            (showDivider ? (
                              <div className="relative py-3 px-3">
                                ...
                                <div className="relative flex justify-center">
                                  <span className="bg-white px-2 text-xs text-slate-500 uppercase tracking-wider">
                                    Already in group
                                  </span>
                                </div>
                              </div>
                          ...
                          <div
                            className={`flex items-center gap-3 p-3 transition-colors ${contact.isJoined ? 'opacity-50 cursor-not-allowed' : 'hover:bg-slate-50 cursor-pointer'} ${selectedIds.has(contact.id) ? 'bg-slate-50' : ''}`}
                          ...
                          >
                            <div className="relative shrink-0">
                              ...
                              {selectedIds.has(contact.id) && (
                                <div className="absolute -bottom-1 -right-1 bg-[#008069] text-white rounded-full p-0.5 border-2 border-white animate-in zoom-in duration-200">
                                  <Check size={10} strokeWidth={4} />
                                </div>
                              )}
                            </div>
                            <div className="flex-1 min-w-0">
                              <h4 className="text-sm font-medium text-slate-800 truncate">{contact.name}</h4>
                              <p className="text-xs text-slate-500 truncate">{contact.phone}</p>
                            </div>
                            <div className={`w-5 h-5 rounded border flex items-center justify-center transition-all duration-200 ${selectedIds.has(contact.id) && !contact.isJoined ? 'bg-[#008069] border-[#008069]' : 'border-slate-300'} ${contact.isJoined ? 'bg-slate-200 border-slate-300' : ''}`}>
                              {selectedIds.has(contact.id) && !contact.isJoined && <Check size={14} className="text-white" />}
                              {contact.isJoined && <Check size={14} className="text-slate-400" />}
                            </div>
                          </div>
                        </div>
                      );
          ...
          <div className="p-4 border-t border-slate-100 bg-white shrink-0">
             <button
                ...
                className="w-full bg-[#008069] text-white py-2.5 rounded-lg font-medium text-sm hover:bg-[#006d59] transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 cursor-pointer"
             >
