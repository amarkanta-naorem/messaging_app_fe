...
export default function ChatMenuModal ({ showMenu, setShowMenu, setShowNewMessage, setShowCreateGroup, setShowProfile}: ChatMenuModalProps) {
    ...
    return (
        <div className="absolute right-0 top-12 bg-(--bg-card) shadow-[0_4px_20px_rgba(0,0,0,0.1) py-2 w-56 z-50 rounded-xl border border-(--border-primary) origin-top-right animate-in fade-in zoom-in-95 duration-200">
          <button className="px-4 py-3 hover:bg-(--bg-hover) cursor-pointer text-(--text-primary) text-[15px] flex items-center gap-3 transition-colors font-medium" onClick={() => {
            setShowMenu(false);
            setShowProfile(true);
          }}>
            {user?.avatar ? (
