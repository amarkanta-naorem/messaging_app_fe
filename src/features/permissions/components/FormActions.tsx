...
export function FormActions({ loading = false, onCancel, isEdit = false }: FormActionsProps) {
  return (
    <div className="flex items-center justify-end gap-3">
      ...
      <Button variant="customBg" size="md" type="submit" loading={loading} className="px-6 py-2.5 rounded-xl font-medium">{isEdit ? "Update Permission" : "Create Permission"}</Button>
    </div>
  );
}