...
export function PermissionList() {
  ...
  const handleCreate = () => {
    ...
    setIsFormOpen(true);
    router.push(`${pathname}?permissionForm=open`);
  };
    ...
    <div className="space-y-6">
      ...
      {isFormOpen ? (
        <PermissionForm
          ...
          onClose={() => { setIsFormOpen(false); setEditingPermission(null); router.push(pathname); }}
          loading={formLoading}
        />
      ) : isDetailsOpen ? (
        <PermissionDetails
          ...
          onClose={() => { setIsDetailsOpen(false); setViewingPermission(null); }}
        />
      ) : (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-sm text-(--text-secondary)">
              ...
              <tbody className="divide-y divide-(--border-primary)">
                {loading ? (
                  <tr>
                    <td colSpan={4} className="px-6 py-8 text-center text-(--text-muted)">Loading permissions...</td>
                  </tr>
                ) : filteredPermissions.length === 0 ? (
                  <tr>
                    <td colSpan={4} className="px-6 py-8 text-center text-(--text-muted)">No permissions found.</td>
                  </tr>
                ) : (
                  filteredPermissions.map((perm) => (
                    <tr key={perm.id} className="group hover:bg-(--bg-hover) transition-colors">
                      ...
                      <td className="px-6 py-4">
                        <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-400">{perm.module}</span>
                      </td>
                      <td className="px-6 py-4 text-right">
                        <div className="flex items-center justify-end gap-2">
                          <button onClick={() => handleView(perm)} className="p-2 rounded-lg text-(--text-muted) bg-(--bg-tertiary)/70 hover:bg-(--bg-hover) hover:text-(--accent-primary) focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-(--accent-primary) focus:ring-offset-(--bg-card) transition-all duration-150 cursor-pointer" title="View Details" aria-label={`View details for ${perm.name}`}>
                            <Eye size={18} />
                          </button>
                          <button onClick={() => handleEdit(perm)} className="p-2 rounded-lg text-(--text-muted) bg-(--bg-tertiary)/70 hover:bg-(--bg-hover) hover:text-blue-500 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 focus:ring-offset-(--bg-card) transition-all duration-150 cursor-pointer" title="Edit" aria-label={`Edit ${perm.name}`}>
                            <SquarePen size={18} />
                          </button>
                          <button onClick={() => setDeleteTarget(perm)} className="p-2 rounded-lg text-(--text-muted) bg-(--bg-tertiary)/70 hover:bg-(--bg-hover) hover:text-red-500 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 focus:ring-offset-(--bg-card) transition-all duration-150 cursor-pointer" title="Delete" aria-label={`Delete ${perm.name}`}>
                            <Trash2 size={18} />
                          </button>
                        </div>
                      </td>
